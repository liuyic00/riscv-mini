...
class DatapathTester(datapath: => Datapath, testType: DatapathTest) extends BasicTester with TestUtils {
  ...
  switch(state) {
    is(sInit) {
      (0 until mini.Const.PC_START by 4).foreach { addr =>
        mem((addr / 4).U) := (if (addr == mini.Const.PC_EVEC + (3 << 6)) fin else nop)
      }
      mem((mini.Const.PC_START / (xlen / 8)).U + cntr) := VecInit(insts)(cntr)
